...
- [ ] **39. Write up pipeline step in-detail document** - Analyze each step and concisely summarize the entire pipeline flow. Data flow: BLE Beacon → ESP32 Anchors → MQTT → Inference Service (Kalman → Features → Particle Filter [HMM → Predict → Update → Resample] → Room Label → Activity → Zone Classifier → DB). Ensure to also touch on how floor conflicts are handled and how to resolve individual service's prediction conflicts (HMM believes 2F while particle believes 1F). Also include a focus on the various techniques used for particle prediction, signal smoothing, how we handled occlusion and attenuation, particle wall detection, particles and stairway interaction, particle spawning across obstacles, etc.